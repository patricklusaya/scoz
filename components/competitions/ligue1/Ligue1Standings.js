...
import { fetchLigue1Standings } from './Ligue1Services';

const Ligue1Standings = ({selectedSeason }) => {
//Initialize states
  ...
  //Layout of the header select right icon
    ...
    useEffect(() => {
      const fetchStandingsData = async () => {
        try {
          const data = await fetchLigue1Standings();
          setStandings(data);
        } catch (error) {
          console.error(error);
        }
   ...
   const fetchStandingsData = async (year) => {
    try {
      const data = await fetchLigue1Standings(year);
      setStandings(data);
    } catch (error) {
      console.error(error);
    }
    ...
    const onRefresh = async (year) => {
        setRefreshing(true);
        try {
          await fetchStandingsData(selectedSeason);
        } catch (error) {
          console.error(error);
        }
        setRefreshing(false);
