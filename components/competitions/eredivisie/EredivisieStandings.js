...
import { fetchEredivisieStandings } from './EredivisieServices';


const EredivisieStandings = ({selectedSeason }) => {
//Initialize states
  ...
  //Layout of the header select right icon
    ...
    useEffect(() => {
      const fetchStandingsData = async () => {
        try {
          const data = await fetchEredivisieStandings();
          setStandings(data);
        } catch (error) {
          console.error(error);
        }
   ...
   const fetchStandingsData = async (year) => {
    try {
      const data = await fetchEredivisieStandings(year);
      setStandings(data);
    } catch (error) {
      console.error(error);
    }
    ...
    const onRefresh = async (year) => {
        setRefreshing(true);
        try {
          await fetchStandingsData(selectedScorerYear);
        } catch (error) {
          console.error(error);
        }
        setRefreshing(false);
...
export default EredivisieStandings;