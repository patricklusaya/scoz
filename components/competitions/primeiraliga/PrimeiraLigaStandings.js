...
import { fetchPPLStandings } from './PrimeiraLigaServices';

const PrimeiraLigaStandings = ({selectedScorerYear,selectedSeason }) => {
//Initialize states
  ...
  //Layout of the header select right icon
    ...
    useEffect(() => {
      const fetchStandingsData = async () => {
        try {
          const data = await fetchPPLStandings();
          setStandings(data);
        } catch (error) {
          console.error(error);
        }
   ...
   const fetchStandingsData = async (year) => {
    try {
      const data = await fetchPPLStandings(year);
      setStandings(data);
    } catch (error) {
      console.error(error);
    }
    ...
    const onRefresh = async (year) => {
        setRefreshing(true);
        try {
          await fetchPPLStandings(selectedScorerYear);
        } catch (error) {
          console.error(error);
        }
        setRefreshing(false);
...
export default PrimeiraLigaStandings;