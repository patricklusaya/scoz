...
import { fetchSerieaStandings } from './SerieaService';

const SerieaStanding = ({selectedScorerYear, selectedSeason }) => {
//Initialize states
  ...
  //Layout of the header select right icon
    ...
    useEffect(() => {
      const fetchStandingsData = async () => {
        try {
          const data = await fetchSerieaStandings();
          setStandings(data);
        } catch (error) {
          console.error(error);
        }
   ...
   const fetchStandingsData = async (year) => {
    try {
      const data = await fetchSerieaStandings(year);
      setStandings(data);
    } catch (error) {
      console.error(error);
    }
    ...
    const onRefresh = async (year) => {
        setRefreshing(true);
        try {
          await fetchEplStandings(selectedScorerYear);
        } catch (error) {
          console.error(error);
        }
        setRefreshing(false);
