...
import { fetchEflStandings } from './EflServices';

const EflStandings = ({selectedSeason }) => {
//Initialize states
  ...
  //Layout of the header select right icon
    ...
    useEffect(() => {
      const fetchStandingsData = async () => {
        try {
          const data = await fetchEflStandings();
          setStandings(data);
        } catch (error) {
          console.error(error);
        }
   ...
   const fetchStandingsData = async (selectedSeason) => {
    try {
      const data = await fetchEflStandings(selectedSeason);
      setStandings(data);
    } catch (error) {
      console.error(error);
    }
    ...
    const onRefresh = async (year) => {
        setRefreshing(true);
        try {
          await fetchEflStandings(selectedScorerYear);
        } catch (error) {
          console.error(error);
        }
        setRefreshing(false);
