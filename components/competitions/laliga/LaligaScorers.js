...
import { fetchLaligaScorers } from "../../../services/matchesServices";
import { useNavigation } from "@react-navigation/native";
import Ionicons from "react-native-vector-icons/Ionicons";
import { ModalContext } from "../../modal/ModalContext";
import { Images } from "../Images";
import CustomLoader from "../../loaders/CustomLoader";
export default function LaligaScorers({ selectedYear, selectedScorerYear }) {
  ...
  useEffect(() => {
    if (selectedYear) {
      if (selectedYear.length === 4) {
        ...
        const formattedYear = endYear.toString();
        console.log('my year', formattedYear);
        getTopScorers(formattedYear);
        setIsLoading(false)
      }
    }
  }, [selectedYear]);
  ...
  //   try {
  //     // Fetch top scorers data for the selected year
  //     const data = await fetchLaligaScorers(year);
  ...
  return (
    <View style={styles.container}>
      ...
      <View style={styles.errorCard}>
        ...
        <Text style={styles.errorText}>Sorry, currently unavailable.</Text>
      </View>
      </View>
    ) : (
      <FlatList
